...
def match_properties_rdflib(obj_first, obj_second):
...
def iterate_over_key_rdflib(obj_first, obj_second, matched, key, dist_function):
    lowest_distance = sys.maxsize
    for pos_match_first in matched[key]['first']:
        for pos_match_second in matched[key]['second']:
...
def calculate_score_rdflib(obj_first, obj_second):
    matched, mismatched = match_properties_rdflib(obj_first, obj_second)
    scores = {}
    for key in matched.keys():
        if key in ("http://schema.org/datePublished", "http://schema.org/dateCreated"):
            lowest_distance = iterate_over_key_rdflib(obj_first, obj_second, matched, key, linear_dist)
        else:
            lowest_distance = iterate_over_key_rdflib(obj_first, obj_second, matched, key, Levenshtein.distance)
        scores[key] = 1 / (1 + lowest_distance)
    score = sum(scores.values()) / (len(matched) + len(mismatched) / 2)
    return {'total_score': score, 'partial_scores': scores}
